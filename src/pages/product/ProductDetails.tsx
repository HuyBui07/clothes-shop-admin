...
export default function ProductDetails(props: any) {
  ...
  const productsChanged = props.productsChanged;
  const setProductsChanged = props.setProductsChanged;
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        console.log(API_CONST + "/product/get-detail/" + selectedProductId);
        await fetch(API_CONST + "/product/get-detail/" + selectedProductId)
          .then((response) => {
            if (!response.ok) {
  ...
  const handleDeleteImageFromCloudinary = async (image: string) => {
    const response = await fetch(
      API_CONST + "/product/delete-image/" + selectedProductId,
      {
        method: "DELETE",
        headers: {
    ...
    stocks.forEach((stock) => {
      stock.sizes = stock.sizes.sort((a, b) => {
        if (commomSizes.indexOf(a.size) < commomSizes.indexOf(b.size))
          return -1;
        if (commomSizes.indexOf(a.size) > commomSizes.indexOf(b.size)) return 1;
        return 0;
      });
    });
  ...
  const handleEditProduct = async () => {
    ...
    const response = await fetch(
      API_CONST + "/product/update/" + selectedProductId,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("adminAccessToken")}`,
            ...
            <div className="relative">
              ...
              <FaTimes
                ...
                onClick={() => {
                  ...
                  handleDeleteImageFromCloudinary(image);
                }}
              />
            </div>
          ))}
            ...
            onChange={(e) => {
              if (e.target.files) {
                ...
                if (totalFiles <= 6) {
                  ...
                  const remainingFiles = totalFiles - (totalFiles - 5);
